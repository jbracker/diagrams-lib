...
width :: (Num b, AdditiveGroup b, Enveloped a, V a ~ V2 b) => a -> Scalar (V2 b)
width = maybe 0 (negate . uncurry (-)) . extentX

-- | Compute the height of an enveloped object.
height :: (Num b, AdditiveGroup b, Enveloped a, V a ~ V2 b) => a -> Scalar (V2 b)
height = maybe 0 (negate . uncurry (-)) . extentY

-- | Compute the width and height of an enveloped object.
size2D :: (Num b, AdditiveGroup b, Enveloped a, V a ~ (V2 b)) => a -> (Scalar (V2 b), Scalar (V2 b))
size2D = width &&& height

-- | Compute the size of an enveloped object as a 'SizeSpec2D' value.
sizeSpec2D :: (Num b, AdditiveGroup b, Enveloped a, V a ~ (V2 b)) => a -> SizeSpec2D (Scalar (V2 b))
...
--   empty envelope.
extentX :: (Num b, AdditiveGroup b, Enveloped a, V a ~ V2 b) => a -> Maybe (Scalar (V2 b), Scalar (V2 b))
...
extentY :: (Num b, AdditiveGroup b, Enveloped a, V a ~ V2 b) => a -> Maybe (Scalar (V2 b), Scalar (V2 b))
...
--   envelope.
center2D :: (Num b, AdditiveGroup b, Enveloped a, V a ~ V2 b) => a -> P2 (Scalar (V2 b))
